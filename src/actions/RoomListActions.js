 ...
 * @see asyncAction
 */
RoomListActions.tagRoom = function(
    matrixClient,
    room,
    oldTag,
    newTag,
    ...
    if (
        ...
        const prevOrder =
            indexBefore <= 0 ? 0 : newList[indexBefore].tags[newTag].order;
        const nextOrder =
            indexAfter >= newList.length
                ? 1
                : newList[indexAfter].tags[newTag].order;
    ...
    return asyncAction(
        ...
        () => {
            ...
            if (
                oldTag &&
                oldTag !== "im.vector.fake.direct" &&
                hasChangedSubLists
            ) {
                const promiseToDelete = matrixClient
                    .deleteRoomTag(roomId, oldTag)
                    .catch(function(err) {
                        ...
                        Modal.createTrackedDialog(
                            "Failed to remove tag from room",
                            "",
                            ErrorDialog,
                            {
                                title: _t(
                                    "Failed to remove tag %(tagName)s from room",
                                    { tagName: oldTag }
                                ),
                                description:
                                    err && err.message
            ...
            if (
                ...
                const promiseToAdd = matrixClient
                    .setRoomTag(roomId, newTag, metaData)
                    .catch(function(err) {
                        const ErrorDialog = sdk.getComponent(
                            "dialogs.ErrorDialog"
                        );
                        console.error(
                            "Failed to add tag " + newTag + " to room: " + err
                        );
                        Modal.createTrackedDialog(
                            "Failed to add tag to room",
                            "",
                            ErrorDialog,
                            {
                                title: _t(
                                    "Failed to add tag %(tagName)s to room",
                                    { tagName: newTag }
                                ),
                                description:
                                    err && err.message
