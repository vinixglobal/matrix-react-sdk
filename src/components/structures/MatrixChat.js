...
export default React.createClass({
    ...
    getInitialState: function() {
        const s = {
            ...
            collapseLhs: false,
            collapsedRhs:
                window.localStorage.getItem("mx_rhs_collapsed") === "true",
            leftDisabled: false,
    ...
    getDefaultHsUrl(defaultToMatrixDotOrg) {
        defaultToMatrixDotOrg =
            typeof defaultToMatrixDotOrg !== "boolean"
                ? true
                : defaultToMatrixDotOrg;
        if (!this.state.defaultHsUrl && defaultToMatrixDotOrg)
    ...
    componentDidMount: function() {
        ...
        Lifecycle.attemptTokenLogin(this.props.realQueryParams).then(
            loggedIn => {
                if (loggedIn) {
        ...
        return Promise.resolve()
            .then(() => {
                return Lifecycle.loadSession({
                    ...
                    defaultDeviceDisplayName: this.props
    ...
    startPageChangeTimer() {
        ...
        if (this._pageChanging) {
            console.warn(
                "MatrixChat.startPageChangeTimer: timer already started"
            );
            return;
        }
    ...
    stopPageChangeTimer() {
        ...
        const measurement = performance
            .getEntriesByName("riot_MatrixChat_page_change_delta")
            .pop();
    ...
    onAction: function(payload) {
        ...
        switch (payload.action) {
            ...
            case "view_user_settings": {
                const UserSettingsDialog = sdk.getComponent(
                    "dialogs.UserSettingsDialog"
                );
                Modal.createTrackedDialog(
                    "User settings",
                    "",
            ...
            case "view_create_group":
                {
                    const CreateGroupDialog = sdk.getComponent(
                        "dialogs.CreateGroupDialog"
                    );
                    Modal.createTrackedDialog(
                        "Create Community",
                        "",
            ...
            case "view_room_directory":
                {
                    const RoomDirectory = sdk.getComponent(
                        "structures.RoomDirectory"
                    );
                    Modal.createTrackedDialog(
                        "Room directory",
                        "",
            ...
            case "view_start_chat_or_reuse":
                this._chatCreateOrReuse(
                    payload.user_id,
                    payload.go_home_on_cancel
                );
                break;
            case "view_create_chat":
                showStartChatInviteDialog();
                break;
            case "view_create_text":
                showTextMessage();
                break;
            case "view_invite":
            ...
            case "panel_disable": {
                this.setState({
                    leftDisabled:
                        payload.leftDisabled || payload.sideDisabled || false,
                    middleDisabled: payload.middleDisabled || false,
                    rightDisabled:
                        payload.rightDisabled || payload.sideDisabled || false
                });
                break;
            }
            case "set_theme":
                ...
                if (shouldLeave) {
                    ...
                    d.then(
                        errors => {
                            ...
                            for (const leftRoomId of Object.keys(errors)) {
                                ...
                                if (
                                    ...
                                    title = _t(
                                        "Can't leave Server Notices room"
                                    );
                                    message = _t(
                        ...
                        err => {
                            ...
                            modal.close();
                            console.error(
                                "Failed to leave room " + roomId + " " + err
                            );
                            Modal.createTrackedDialog(
                                "Failed to leave room",
                                "",
    ...
    _onWillStartClient() {
        ...
        cli.on("sync.unexpectedError", function(err) {
            if (
                err.message &&
                err.message.includes("live timeline ") &&
                err.message.includes(" is no longer live ")
            ) {
                console.error(
                    "Caught timeline explosion - trying to ask user for more information"
                );
                if (Modal.hasDialogs()) {
                    console.warn(
                        "User has another dialog open - skipping prompt"
                    );
                    return;
                }
                Modal.createTrackedDialog(
    ...
    handleResize: function(e) {
        ...
        if (
            this._windowWidth > hideLhsThreshold &&
            window.innerWidth <= hideLhsThreshold
        ) {
            dis.dispatch({ action: "hide_left_panel" });
        }
        if (
            this._windowWidth <= showLhsThreshold &&
            window.innerWidth > showLhsThreshold
        ) {
            dis.dispatch({ action: "show_left_panel" });
        }
        if (
            this._windowWidth > hideRhsThreshold &&
            window.innerWidth <= hideRhsThreshold
        ) {
            dis.dispatch({ action: "hide_right_panel" });
        }
        if (
            this._windowWidth <= showRhsThreshold &&
            window.innerWidth > showRhsThreshold
        ) {
    ...
    _tryDiscoverDefaultHomeserver: async function(serverName) {
        try {
            ...
            if (state !== AutoDiscovery.SUCCESS) {
                console.error(
                    "Failed to discover homeserver on startup:",
                    discovery
                );
                this.setState({
                    defaultServerDiscoveryError:
                        discovery["m.homeserver"].error,
            ...
            this.setState({
                defaultServerDiscoveryError: _t(
                    "Unknown error discovering homeserver"
                ),
                loadingDefaultHomeserver: false
            });
        }
    ...
    render: function() {
        ...
        if (this.state.view === VIEWS.POST_REGISTRATION) {
            const PostRegistration = sdk.getComponent(
                "structures.auth.PostRegistration"
            );
            return (
        ...
        if (this.state.view === VIEWS.LOGGED_IN) {
            ...
            const isStoreError =
                ...
                this.state.syncError instanceof Matrix.InvalidStoreError;
            ...
            if (this.state.ready && this.state.page_type && !isStoreError) {
                 ...
                 */
                const LoggedInView = sdk.getComponent(
                    "structures.LoggedInView"
                );
                return (
                    <LoggedInView
        ...
        if (this.state.view === VIEWS.REGISTER) {
            ...
            return (
                <Registration
                    ...
                    defaultServerDiscoveryError={
                        this.state.defaultServerDiscoveryError
                    }
        ...
        if (this.state.view === VIEWS.FORGOT_PASSWORD) {
            const ForgotPassword = sdk.getComponent(
                "structures.auth.ForgotPassword"
            );
            return (
                <ForgotPassword
                    defaultServerName={this.getDefaultServerName()}
